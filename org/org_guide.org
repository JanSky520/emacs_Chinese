...
#+end_src
** 激活
将以下几行添加到您的 Emacs 初始化文件中，为三个命令定义全局键，这些键在任何 Emacs 缓冲区（而不仅仅是 Org 缓冲区）
中都很有用。可根据自身习惯自行选择合适的键。
#+begin_src emacs-lisp
...
+ C-c C-f (org-backward-heading-same-level) 跳转到下一个同级标题。  
+ C-c C-b (outline-backward-same-level) 跳转到上一个同级标题。  
+ C-c C-u (outline-up-heading) 向上跳转到更高层级的标题。
** 结构编辑
